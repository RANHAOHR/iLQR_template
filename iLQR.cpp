...
ILQR::ILQR() {
    ...
    Q_T[0][0] = 5000; Q_T[1][1] = 3000; Q_T[2][2] = 5000;
    ...
    lambda = 1;
    lambda_factor = 1.6;
    lambda_tol = 1e-3; //haven't used yet
...
void ILQR::value_iteration(std::vector< std::vector<double> > &x_t_stack, std::vector< std::vector<double> > &u_t_stack){
    ...
    for (int cnt = 0; cnt < itr; ++cnt) {
        ...
        //forward propagation
        double diff_cost = 0;
        std::vector<std::vector<double> > x_stack_new; x_stack_new.resize(N_path);
        for (int iS = 0; iS < D_state; ++iS)
        {
            x_stack_new[0].resize(D_state);
            x_stack_new[0][iS] = x_t_stack[0][iS];
        }
        ...
        for (int ipath = 0; ipath < N_path-1; ++ipath) {
            ...
            std::vector<double> dx = diff_vector(x_stack_new[ipath], x_t_stack[ipath]);
            std::vector<double> temp = left_Mult_AB(k_2_mat, dx);
            for (int i = 0; i < D_control; ++i) {
                u_t_stack[ipath][i] = u_t_stack[ipath][i]+ K_1[i][ipath] + temp[i];
            }

            std::vector<double> x_nextstep = dynamic_function(x_stack_new[ipath], u_t_stack[ipath]) ;

            diff_cost += norm(x_nextstep, x_t_stack[ipath+1]);
            for (int iS = 0; iS < D_state; ++iS)
            {
                x_stack_new[ipath+1].resize(D_state);
                x_stack_new[ipath+1][iS] = x_nextstep[iS];
            }
        ...
        for (int ipath = 0; ipath < N_path; ++ipath) {
            for (int iS = 0; iS < D_state; ++iS)
            {
                x_t_stack[ipath][iS] = x_stack_new[ipath][iS];
            }
        }
